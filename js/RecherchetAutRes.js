function TransformerChamp(id) {
	...
	Form.setAttribute ("action","GestionAutoritesResponsables");
	var NomForm="form_modif_nom_aut_res"+id;
	Form.setAttribute ("id",NomForm);

	//On crée un input caché pour le numero du boitier
	var HiddenInput = document.createElement("input");
	HiddenInput.setAttribute ("type","hidden");
	HiddenInput.setAttribute ("name","id_autres");
	HiddenInput.setAttribute ("id","id_autres");
	HiddenInput.setAttribute ("value",id);
	Form.append(HiddenInput);

	//On crée un select
	var Select = document.createElement("select");
	Select.setAttribute ("name","modif_nom_aut_res");
